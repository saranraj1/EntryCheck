"""
ExplainCheck — Cosine Similarity stability metric (Stage 4, DR-003A).
...
Scientific definition:
    ...
    Cosine_similarity(A, A') = (A · A') / (||A|| * ||A'||)

    When both attribution vectors are zero, similarity = 1.0 (identical degenerate attributions).
...
def cosine_similarity_pair(a: np.ndarray, b: np.ndarray) -> float:
    """
    Cosine similarity between two dense attribution vectors.

    Edge cases:
    - Both zero → 1.0 (identical degenerate attributions)
    - One zero, other nonzero → 0.0
    ...
    ------
    ValueError
        If arrays have different shapes or contain NaN/Inf.
    """
    if a.shape != b.shape:
